...
class ZenForTokenClassificationCRF(ZenPreTrainedModel):
    """ZEN model for token-level classification.
    This module is composed of the ZEN model with a linear layer on top of
    the full hidden state of the last layer.
    ...
    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False):
        super(ZenForTokenClassificationCRF, self).__init__(config)
        self.output_attentions = output_attentions
        self.num_labels = num_labels
        self.bert = ZenModel(config, output_attentions=output_attentions,
                              keep_multihead_output=keep_multihead_output)
...
class ZenForTokenClassificationBiLSTMCRF(ZenPreTrainedModel):
    """ZEN model for token-level classification.
    This module is composed of the ZEN model with a linear layer on top of
    the full hidden state of the last layer.
    ...
    def __init__(self, config, num_labels=2, lstm_hidden_size=200, lstm_num_layers=1, output_attentions=False, keep_multihead_output=False):
        super(ZenForTokenClassificationBiLSTMCRF, self).__init__(config)
        self.output_attentions = output_attentions
        self.num_labels = num_labels
        self.bert = ZenModel(config, output_attentions=output_attentions,
                              keep_multihead_output=keep_multihead_output)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.lstm = BiLSTM(input_size=config.hidden_size, hidden_size=lstm_hidden_size,num_layers=lstm_num_layers,batch_first=True)
